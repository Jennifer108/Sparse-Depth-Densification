...
def evaluateError(output, target):
    ...
    if (nValidElement.data.cpu().numpy() > 0):
        ...
        nanMask = getNanMask(realMatrix)
        realMatrix[nanMask] = 0

        
        nValidElement = torch.where(torch.isinf(nValidElement), torch.full_like(nValidElement, 0), nValidElement)
        realMatrix = torch.where(torch.isinf(realMatrix), torch.full_like(realMatrix, 0), realMatrix)
        ...
        zOverY = torch.where(torch.isinf(zOverY), torch.full_like(zOverY, 0), zOverY)
        yOverZ = torch.where(torch.isinf(yOverZ), torch.full_like(yOverZ, 0), yOverZ)
